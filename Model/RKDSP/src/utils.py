...
def PPR_lnc(node2neigh_list, alpha=0.15):
    sim_matrix_list = []
    adjs = []
    for node2neigh in node2neigh_list:
        ...
        A = np.ones(shape=len(node2neigh))
        I = np.diag(A)
        adjs.append(adj)
        sim_matrix_list.append(
            alpha * np.linalg.inv((I - (1 - alpha) * (adj / np.sum(adj, axis=1).reshape(1, -1)))).astype(np.float32))
    return adjs, sim_matrix_list


def PPR_drug(node2neigh_list, alpha=0.15):
    sim_matrix_list = []
    adjs = []
    for node2neigh in node2neigh_list:
        adj = []
...
def PPR_dis(node2neigh_list, alpha=0.15):
    sim_matrix_list = []
    adjs = []
    for node2neigh in node2neigh_list:
        adj = []
        for node in range(len(node2neigh)):
            neighs = np.zeros(shape=len(node2neigh), dtype=np.int32)
            dis = np.full((405, 1), 240)
            # print(len(node2neigh))
            # print(node2neigh[node+240])
            # print(neighs.shape)
            neighs[node2neigh[node + 240] - dis] = 1
            neighs[node] = 1
            adj.append(neighs.reshape(1, -1))
...
def PPR_se(node2neigh_list, alpha=0.15):
    sim_matrix_list = []
    adjs = []
    for node2neigh in node2neigh_list:
        adj = []
        for node in range(len(node2neigh)):
            neighs = np.zeros(shape=len(node2neigh), dtype=np.int32)
            se = np.full((4192, 1), 708)
            # print(len(node2neigh))
            # print(node2neigh[node])
            # print(neighs.shape)
            neighs[node2neigh[node + 708] - se] = 1
            neighs[node] = 1
            adj.append(neighs.reshape(1, -1))
...
def get_topk_neigh_lnc(adjs, node, topk, node2neigh_list):
    """
...
def get_topk_neigh_single_lnc(data):
    adjs, node, topk, node2neigh_list = data
    return get_topk_neigh_lnc(adjs, node, topk, node2neigh_list)


def get_topk_neigh_dis(adjs, node, topk, node2neigh_list):
    """
    Get the node TOPK neighbors and rebuild the subgraph
    """
    result = []
    for idx, adj in enumerate(adjs):
        node2neigh = node2neigh_list[idx]
        pairs = [(adj[i], i + 240) for i in range(len(adj))]
        pairs[node - 240] = (-100, node)
        if len(node2neigh[node]) <= 1:
            topk_nodes = [node] * (topk + 1)
        else:
            topk_nodes = [node] + find_topk(pairs, topk)
...
def get_topk_neigh_se(adjs, node, topk, node2neigh_list):
    """
    Get the node TOPK neighbors and rebuild the subgraph
    """
    result = []
    for idx, adj in enumerate(adjs):
        node2neigh = node2neigh_list[idx]
        pairs = [(adj[i], i + 708) for i in range(len(adj))]
        pairs[node - 708] = (-100, node)
        if len(node2neigh[node]) <= 1:
            topk_nodes = [node] * (topk + 1)
        else:
            topk_nodes = [node] + find_topk(pairs, topk)
...
def get_topk_neigh_single_dis(data):
    adjs, node, topk, node2neigh_list = data
    return get_topk_neigh_dis(adjs, node, topk, node2neigh_list)


def get_topk_neigh_single_se(data):
    adjs, node, topk, node2neigh_list = data
    return get_topk_neigh_se(adjs, node, topk, node2neigh_list)
...
def get_topk_neigh_multi_lnc(target_nodes, node2neigh_list, topk, adjs, Ss):
    data = []
    for node in range(len(target_nodes)):
        data.append([[adj[node] for adj in Ss], node, topk, node2neigh_list])
    pool = Pool(10)
    topk_result = pool.map(get_topk_neigh_single_lnc, data)  # kanyixia 这里是属性还是什么
    pool.close()
    pool.join()
    ...
    for new_adj in new_adjs:
        adj_result.append(get_topk_adj_transfer(new_adj))  # kanyixia 这里是属性还是什么
...
def get_topk_neigh_multi_drug(target_nodes, node2neigh_list, topk, adjs, Ss):
    data = []
    for node in range(len(target_nodes)):
        data.append([[adj[node] for adj in Ss], node, topk, node2neigh_list])
    pool = Pool(10)
...
def get_topk_neigh_multi_dis(target_nodes, node2neigh_list, topk, adjs, Ss):
    data = []
    for node in range(len(target_nodes)):
        data.append([[adj[node] for adj in Ss], node + 240, topk, node2neigh_list])
    pool = Pool(10)
    topk_result = pool.map(get_topk_neigh_single_dis, data)  # kanyixia 这里是属性还是什么
    pool.close()
    pool.join()
    ...
    for topk_list in (topk_result):
        temp = []
        for idx, topk_neigh in enumerate(topk_list):
            topk_neigh = list(map(lambda x: x - 240, topk_neigh))
            new_adj = adjs[idx][topk_neigh][:, topk_neigh]
            temp.append(new_adj)
        new_adjs.append(temp)
    ...
    for idx in range(len(topk_result)):
        ...
        for idy in range(len(topk_result[idx])):
            node2neigh = node2neigh_list[idy]
            # a = node2neigh[idx]
            status.append(0 if len(node2neigh[idx + 240]) <= 1 else 1)
            temp.append([np.array(topk_result[idx][idy]), adj_result[idx][idy]])
        final_result.append([idx + 240, status, temp])
    return final_result


def get_topk_neigh_multi_se(target_nodes, node2neigh_list, topk, adjs, Ss):
    data = []
    for node in range(len(target_nodes)):
        data.append([[adj[node] for adj in Ss], node + 708, topk, node2neigh_list])
    pool = Pool(10)
    topk_result = pool.map(get_topk_neigh_single_se, data)  # kanyixia 这里是属性还是什么
    pool.close()
    pool.join()
    ...
    for topk_list in (topk_result):
        temp = []
        for idx, topk_neigh in enumerate(topk_list):
            topk_neigh = list(map(lambda x: x - 708, topk_neigh))
            new_adj = adjs[idx][topk_neigh][:, topk_neigh]
            temp.append(new_adj)
        new_adjs.append(temp)
    ...
    for idx in range(len(topk_result)):
        ...
        for idy in range(len(topk_result[idx])):
            node2neigh = node2neigh_list[idy]
            # a = node2neigh[idx]
            status.append(0 if len(node2neigh[idx + 708]) <= 1 else 1)
            temp.append([np.array(topk_result[idx][idy]), adj_result[idx][idy]])
        final_result.append([idx + 708, status, temp])
    return final_result
...
def load_node(path):
    ...
    with open(path) as f:
        for line in f:
            ...
            # node2attr[int(data[0])]=np.array([i for i in data[2].split(',')])

    return node2attr


def load_data_lnc(ltypes, base_path, use_features):
    name2id = load_node2new_id(os.path.join(base_path, 'node2id_lnc.txt'))
    ...
    for ltype in ltypes:
        ...
        print(os.path.join(base_path, f'sub_graph_{ltype}.edgelist'))
...
def load_data_drug(ltypes, base_path, use_features):
    name2id = load_node2new_id(os.path.join(base_path, 'node2id_drug.txt'))
    id2name = {v: k for k, v in name2id.items()}
    node2neigh_list = []
    print(id2name)
    pos_edges = set()
...
def load_data_dis(ltypes, base_path, use_features):
...
def load_data_se(ltypes, base_path, use_features):
    name2id = load_node2new_id(os.path.join(base_path, 'node2id_se.txt'))
    id2name = {v: k for k, v in name2id.items()}
    node2neigh_list = []
    print(id2name)
    pos_edges = set()
...
def output(args, embeddings, id2name, need_handle=True):
    ...
    output_path = f'../data/{args.dataset}/dis.dat'
    with open(output_path, 'w') as file:
        ...
        for nid, name in id2name.items():
            file.write('{}\t{}\n'.format(name, ' '.join([str(i) for i in embeddings[nid - 708]])))