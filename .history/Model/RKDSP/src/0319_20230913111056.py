...
def main():
    ...
    # drug_se_ass = np.loadtxt("mat_drug_se.txt")
    # se_se_sim = cosine(drug_se_ass.T)
    # se_se_sim[np.isnan(se_se_sim)] = 0
    # se_se_sim = np.array(se_se_sim)
    # drug_drug_sim = np.loadtxt("Similarity_Matrix_Drugs.txt")
    # drug_drug_sim_dis = np.loadtxt("drug_drug_sim_dis.txt")
    # row_drug = np.concatenate((drug_drug_sim, drug_se_ass), axis=1)  # 首尾拼接形成mirna的属性
    # row_se = np.concatenate((drug_se_ass.T, se_se_sim), axis=1)  # 首尾拼接形成dis的属性
    # feature_ds = np.concatenate((row_drug, row_se), axis=0)
    #
    # row_ddd = np.concatenate((sim_matrix_list_drug[0], drug_se_ass), axis=1)  # 首尾拼接形成ddd的属性
    # row_dddse = np.concatenate((drug_se_ass.T, se_se_sim), axis=1)  # 首尾拼接形成dis的属性
    # feature_ddd = np.concatenate((row_ddd, row_dddse), axis=0)
    # np.savetxt("feature_ddd.txt", feature_ddd)
    feature_ddd = np.loadtxt("feature_ddd.txt")
    feature_ddd = torch.tensor(feature_ddd)

    # row_dsd = np.concatenate((sim_matrix_list_drug[1], drug_se_ass), axis=1)  # 首尾拼接形成dsd的属性
    # row_dsdse = np.concatenate((drug_se_ass.T, se_se_sim), axis=1)  # 首尾拼接形成dis的属性
    # feature_dsd = np.concatenate((row_dsd, row_dsdse), axis=0)
    # np.savetxt("feature_dsd.txt", feature_dsd)
    feature_dsd = np.loadtxt("feature_dsd.txt")
    feature_dsd = torch.tensor(feature_dsd)


    # row_sds = np.concatenate((drug_drug_sim, drug_se_ass), axis=1)  # 首尾拼接形成sds的属性
    # row_sdsse = np.concatenate((drug_se_ass.T, sim_matrix_list_se[0]), axis=1)  # 首尾拼接形成dis的属性
    # feature_sds = np.concatenate((row_sds, row_sdsse), axis=0)
    # np.savetxt("feature_sds.txt", feature_sds)
    feature_sds = np.loadtxt("feature_sds.txt")
    feature_sds = torch.tensor(feature_sds)

    # row_sss = np.concatenate((drug_drug_sim, drug_se_ass), axis=1)  # 首尾拼接形成sss的属性
    # row_sssse = np.concatenate((drug_se_ass.T, sim_matrix_list_se[1]), axis=1)  # 首尾拼接形成dis的属性
    # feature_sss = np.concatenate((row_sss, row_sssse), axis=0)
    # np.savetxt("feature_sss.txt", feature_sss)
    feature_sss = np.loadtxt("feature_sss.txt")
    feature_sss = torch.tensor(feature_sss)

    print('load adj finish', datetime.now())
    total_train_views_drug, top_k_d = get_topk_neigh_multi_drug(target_nodes_drug, node2neigh_list_drug, args.topk, adjs_drug, sim_matrix_list_drug)
    total_train_views_se, top_k_s = get_topk_neigh_multi_se(target_nodes_se, node2neigh_list_se, args.topk, adjs_se, sim_matrix_list_se)
    ...
    print(f'sample finish:{datetime.now()}')
    # out_dim = args.dim
    # model = RKDSP(dim_lnc, out_dim, layers=args.layers)
    model = nn.Sequential(
        global_EncodingModule(4900, 400, 200, 0.2),
        FC())
    model = model.cuda()
    ...
    for test in range(5):
        ...
        for epoch in range(args.epochs):
            ...
            for step, (x, train_label) in enumerate(train_loader):  # 从训练集中取出每批训练数据
                ...
                for j in range(x.shape[0]):  # 对每对数据操作形成drug-se成对属性嵌入
                    temp_save1 = []
                    temp_save2 = []
                    temp_saved1 = []
                    temp_saves = []
                    ...
                    x_A = int(x[j][0])
                    y_A = int(x[j][1])
                    # row_1 = np.concatenate((drug_drug_sim[x_A], new_drug_se[x_A],), axis=0)  # 首尾拼接形成drug的属性
                    # row_2 = np.concatenate((new_drug_se.T[y_A], se_se_sim[y_A],), axis=0)  # 首尾拼接形成se的属性
                    # row_3 = np.concatenate((drug_drug_sim_dis[x_A], drug_se_ass[x_A]),axis=0)  # 首尾拼接形成drug的属性
                    temp_saved1.append(x_A)
                    temp_saves.append(y_A + 708)
                    # temp_saved2.append(row_3)
                    # temp_save2.append(row_3)
                    # temp_save2.append(row_2)
                    temp_saved1 = np.array(temp_saved1)
                    temp_saved2 = np.array(temp_saved2)
                    temp_saves = np.array(temp_saves)
                    # temp_saved1 = temp_saved1.reshape(1, 256)
                    # temp_saved2 = temp_saved2.reshape(1, 256)
                    # temp_saves = temp_saves.reshape(1, 256)
                    drug_att1.append(temp_saved1)
                    # drug_att2.append(temp_saved2)
                    se_att.append(temp_saves)
                    # drug_se_attr1.append(temp_save1)  # 形成成对属性嵌入[1,2,4900]
                    # drug_se_attr2.append(temp_save2)  # 形成成对属性嵌入[1,2,4900]

                    topk_ddd.append(top_k_d[x_A][0])
                    topk_dsd.append(top_k_d[x_A][1])
                    topk_sds.append(top_k_s[y_A][0])
                    topk_sss.append(top_k_s[y_A][1])
                feature_ddd = torch.tensor(feature_ddd, device=device)
                feature_dsd = torch.tensor(feature_ddd, device=device)
                feature_sds = torch.tensor(feature_ddd, device=device)
                feature_sss = torch.tensor(feature_ddd, device=device)

                ddd_h, ddd_l, ddd_p, ddd_p_n, dsd_h, dsd_l, dsd_p, dsd_p_n, sds_h, sds_l, sds_p, sds_p_n, sss_h, sss_l, sss_p, sss_p_n = model[0](feature_ddd, feature_dsd, feature_sds, feature_sss, drug_att1, se_att, topk_ddd, topk_dsd, topk_sds, topk_sss)  # 成对属性输入进cnn中训练

                # local_loss_drug_ddd = score(criterion, emb_list_drug, graph_emb_list_drug, [i[1] for i in step_train_views_drug_sample])
                drug_h_list.append(ddd_h)
                drug_h_list.append(dsd_h)
                drug_l_list.append(ddd_l)
                drug_l_list.append(dsd_l)
                drug_p_list.append(ddd_p)
                drug_p_list.append(dsd_p)
                drug_np_list.append(ddd_p_n)
                drug_np_list.append(dsd_p_n)

                se_h_list.append(sds_h)
                se_h_list.append(sss_h)
                se_l_list.append(sds_l)
                se_l_list.append(sss_l)
                se_p_list.append(sds_p)
                se_p_list.append(sss_p)
                se_np_list.append(sds_p_n)
                se_np_list.append(sss_p_n)

                local_loss_drug = score(criterion, drug_h_list, drug_l_list, [i[1] for i in step_train_views_drug_sample])

                global_loss_drug = global_score(criterion, drug_h_list, drug_p_list, drug_np_list,
                                           [i[1] for i in step_train_views_drug_sample])

                local_loss_se = score(criterion, se_h_list, se_l_list, [i[1] for i in step_train_views_se_sample])

                global_loss_se = global_score(criterion, se_h_list, se_p_list, se_np_list,
                                           [i[1] for i in step_train_views_se_sample])

                out = model[1](ddd_h, dsd_h, sds_h, sss_h)
                train_label1.extend(train_label)  # 存储每数据批的标签
                y = torch.LongTensor(np.array(train_label1).astype(int64))  # 转化为Tensor
                y = Variable(y).cuda()  # tensor变成variable之后才能进行反向传播求梯度
                ...
                # feature_ds = torch.tensor(feature_ds, device=device)
                # print(torch.cuda.memory_allocated())

                # print(torch.cuda.memory_allocated())
                loss = loss_func(out, y)  # 计算损失函数
                loss_each = local_loss_drug + global_loss_drug + local_loss_se + global_loss_se
                loss_all = loss + loss_each
                optimizer.zero_grad()  # 梯度清零
                loss_all.backward()  # 反向传播
                optimizer.step()  # 梯度优化
                train_loss += float(loss_all.item())
                # 计算准确率
                _, pred = out.max(1)  # 取最大值的索引，得到预测标签
                num_correct = (pred == y).sum().item()  # 计算每批预测正确的数目
                acc = num_correct / x.shape[0]  # 没批预测正确率
                train_acc += acc  # 总正确率
                num_correct_num += num_correct
        ...
        torch.save(model.state_dict(), "../data/save/RKDSP_0305night_%d.pth" % test)
        print("RKDSP模型参数保存成功")

        #####################测试模型###############################
        model.load_state_dict(torch.load("../data/save/RKDSP_0305night_%d.pth" % test))
        ...
        for test_x, test_label in test_loader:
            ...
            y_A = list(test_x[i][1] for i in range(test_x.shape[0]))  # dis id 0-404 6, 1, 1, 1
            ...
            for j in range(test_x.shape[0]):  # 对每对数据操作形成drug-se成对属性嵌入
                temp_save1 = []
                temp_save2 = []
                temp_saved1 = []
                temp_saves = []
            ...
            ddd_h, ddd_l, ddd_p, ddd_p_n, dsd_h, dsd_l, dsd_p, dsd_p_n, sds_h, sds_l, sds_p, sds_p_n, sss_h, sss_l, sss_p, sss_p_n = \
                model[0](feature_ddd, feature_dsd, feature_sds, feature_sss, drug_att1, se_att, topk_ddd, topk_dsd,
                         topk_sds,
                         topk_sss)  # 成对属性输入进cnn中训练

            pre = model[1](ddd_h, dsd_h, sds_h, sss_h)
