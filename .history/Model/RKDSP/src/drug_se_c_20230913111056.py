...
def main():
    ...
    for test in range(5):
        ...
        np.savetxt("data/train_data.txt", np.array(train_data))
        test_data = np.vstack((sum_list[test], test_zero_postive_last))
        test_data = np.array(test_data)
        print("test", test_data.shape)
        np.savetxt("data/test_data.txt", np.array(test_data))
        ...
        for epoch in range(args.epochs):
            ...
            for step, (x, train_label) in tqdm(enumerate(train_loader)):
                ...
                for channel in range(len(node2neigh_list_drug)):  # 0, 1, 2, 3
                    ...
                    emb_drug, graph_emb_drug = model[0](train_features_drug, train_adj_drug)
                ...
                for channel in range(len(node2neigh_list_se)):  # 0, 1, 2, 3
                    ...
                    emb_se, graph_emb_se = model[1](train_features_se, train_adj_se)
                ...
                emb_list_drug = sum(emb_list_drug)
                emb_list_se = sum(emb_list_se)
