...
def parse_args():
    ...
    parser.add_argument('-model', required=True, type=str, help='Targeting model.',
                        choices=['metapath2vec-ESim', 'PTE', 'HIN2Vec', 'AspEm', 'HEER', 'R-GCN', 'HAN', 'MAGNN', 'HGT',
                                 'TransE', 'DistMult', 'ComplEx', 'ConvE','CKD','HDGI','OpenNe','NSHE'])
...
def check(args):
    if args.attributed == 'True':
        if args.model not in ['R-GCN', 'HAN', 'MAGNN', 'HGT','CKD','HDGI','NSHE']:
...
def main():
    ...
    elif args.model=='CKD': ckd_link_convert(args.dataset, args.attributed, args.version, False)
